...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (SEXT(BITS(src1, 31, 0) ,32) * SEXT(BITS(src2, 31, 0) ,32))>>XLEN);//printf("src1:%#08x\nsrc2:%#08x\nsrc1*src2>>32:%#08llx\n",src1,src2,((ULL)src1 * src2));
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", sra   , R, R(rd) = src1 >> src2);
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = (signed)BITS(src1,31,0) >> SEXT(BITS(imm ,5,0), 6));
